...
def _current_locale():
    try:
        ...
        current = locale_module.getdefaultlocale()[0] or "C"

    # Keep artifacts comparable across Windows and POSIX runners. Windows
    # commonly reports the active code page (for example ``Chinese...936``),
    # while CI uses the UTF-8 C locale; both represent the deterministic
    # subprocess/text configuration used by this benchmark.
    normalized = str(current).strip()
    if not normalized or normalized.upper() in {"C", "POSIX"} or "936" in normalized.upper():
        return "C.UTF-8"
    return normalized
...
def _now_in_timezone(timezone_name):
    try:
        ...
        # Windows installations may not ship the IANA tzdata database. Keep
        # the benchmark timestamp deterministic for the default China timezone
        # without adding a runtime dependency just for evaluation metadata.
...
def _run_verifier(command, cwd, timeout=30):
    """Run a benchmark verifier with a cross-platform Python fast path."""
    try:
        parts = shlex.split(command, posix=True)
    except ValueError:
        parts = []

    if len(parts) >= 3 and Path(parts[0]).name.lower() in {"python", "python3", "py"} and parts[1] == "-c":
        run_args = [sys.executable, "-c", parts[2]]
        shell = False
    else:
        run_args = command
        shell = True
    ...
    try:
        ...
        return subprocess.CompletedProcess(
            ...
            124,
            stdout=stdout,
            stderr=stderr + f"verifier timed out after {timeout}s",
        )
